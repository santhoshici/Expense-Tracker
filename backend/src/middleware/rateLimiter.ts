...
function wrap(impl) {
  return {
    async get(key) { return impl.get(key); },
    async set(key, value, options) { return impl.set(key, value, options); },
    async del(key) { return impl.del(key); },
    async incr(key) { return impl.incr(key); },
  };
}
